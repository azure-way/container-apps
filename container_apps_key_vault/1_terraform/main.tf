...
resource "null_resource" "build_docker_image" {
  provisioner "local-exec" {
    command = "az acr build --registry ${module.container_registry.name} --image ${var.image_name} ../2_samples/ContanerAppsSecretDemo"
  }
...
resource "azurerm_container_app" "sampleapi" {
  ...
  secret {
    name = "response-text"
    key_vault_secret_id = azurerm_key_vault_secret.secret_1.versionless_id
    identity = azurerm_user_assigned_identity.ca_identity.id 
  }
